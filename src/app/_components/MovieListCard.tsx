...
export default async function MovieListCard({
  ...
  return (
    <div className="flex justify-center items-center">
      <span
        key={results.id}
        className="md:w-full mr-6 h-full sm:w-48 sm:grid-cols-3"
      >
        <span key={results.id}>
          <Link
          ...
          >
            ...
            <p
              key={results.id}
              className="pt-4 mr-6 w-36 overflow-visible text-center text-sm font-bold text-gray-900 hover:underline transition-all duration-200 ease-in-out sm:ml-8"
            >
              {results.title}
            </p>
          </Link>
